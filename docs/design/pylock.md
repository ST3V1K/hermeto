...
Pylock uses the same default behavior as requirements.txt: source-only mode.

Rationale:

- Consistent behavior across pip package inputs
- Accurate SBOM generation from source distributions
- Users can opt into prefer-binary mode with explicit `binary` filters if needed

Implementation:

```python
def _download_pylock_dependencies(..., binary_filters=None):
    # binary_filters=None means source-only mode (same as requirements.txt)
    # Users can provide explicit binary filters to enable prefer-binary mode
```

#### 2. Rust Extension Handling

Pylock handles Rust extensions the same way as requirements.txt: vendor Cargo
dependencies from source distributions, and fail if Cargo.lock is out of sync.

Behavior:

- Download source distributions (sdists) by default
- Detect Rust extensions (setuptools-rust, ruff)
- Vendor Cargo dependencies with `cargo vendor --locked`
- Fail if Cargo.lock doesn't match Cargo.toml
- Include Rust dependencies in SBOM

This behavior is inherited from the existing pip infrastructure and requires no
changes for pylock support.
