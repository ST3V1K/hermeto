...
class TestParameters:
    ...
    expected_error: ExitError | None = None
    expected_output: str = ""
...
def fetch_deps_and_check_output(
    ...
    expected_exit_code = (
        int(test_params.expected_error) if test_params.expected_error is not None else 0
    )
    assert exit_code == expected_exit_code, (
        f"Fetching deps ended with unexpected exitcode: {_fmt(exit_code)}"
        f" != {_fmt(expected_exit_code)}, output-cmd: {output}"
    )
