...
@mock.patch("hermeto.core.package_managers.python.pip.main._download_lockfiles")
@mock.patch("hermeto.core.package_managers.python.pip.main._get_pip_metadata")
def test_resolve_pip_pylock_autodiscovered_without_explicit_path(
    mock_metadata: mock.Mock, mock_download: mock.Mock, rooted_tmp_path: RootedPath
) -> None:
    """packaging_tool=pylock alone discovers pylock.toml, not requirements.txt."""
    mock_metadata.return_value = ("foo", "1.0")
    mock_download.return_value = ([], [])
    rooted_tmp_path.join_within_root("pylock.toml").path.write_text('lock-version = "1.0"\n')
    rooted_tmp_path.join_within_root("requirements.txt").path.write_text("")
    ...
    lockfile_type, resolved_lockfiles = mock_download.call_args_list[0].args[:2]
    assert lockfile_type is pip._LOCKFILE_TYPES[PythonPackagingTool.PYLOCK]
    assert [f.path.name for f in resolved_lockfiles] == ["pylock.toml"]


@mock.patch("hermeto.core.package_managers.python.pip.main._download_lockfiles")
@mock.patch("hermeto.core.package_managers.python.pip.main._get_pip_metadata")
def test_resolve_pip_lockfile_filename_selects_requirements(
    mock_metadata: mock.Mock, mock_download: mock.Mock, rooted_tmp_path: RootedPath
) -> None:
    """A *.txt lockfile is treated as requirements, and that exact file is used."""
    mock_metadata.return_value = ("foo", "1.0")
    mock_download.return_value = ([], [])
    rooted_tmp_path.join_within_root("custom-reqs.txt").path.write_text("")

    pip._resolve_pip(
        package_path=rooted_tmp_path,
        output_dir=rooted_tmp_path.join_within_root("output"),
        lockfile=Path("custom-reqs.txt"),
    )

    lockfile_type, resolved_lockfiles = mock_download.call_args_list[0].args[:2]
    assert lockfile_type is pip._LOCKFILE_TYPES[PythonPackagingTool.REQUIREMENTS]
    assert [f.path.name for f in resolved_lockfiles] == ["custom-reqs.txt"]
