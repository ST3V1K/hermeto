...
def _download_dependencies(
    ...
    """
    vcs_deps = [d for d in deps if isinstance(d, VCSPackage)]
    url_deps = [d for d in deps if isinstance(d, URLPackage)]
    pypi_deps = [d for d in deps if isinstance(d, PyPIPackage)]

    processed: list[PipPackage] = []

    for vcs_dep in vcs_deps:
        log.info("-- Processing %s", vcs_dep.name)
        vcs_dep.download(pip_deps_dir)
        processed.append(vcs_dep)
        log.info("-- Finished processing %s", vcs_dep.name)

    for url_dep in url_deps:
        log.info("-- Processing %s", url_dep.name)
        url_dep.download(pip_deps_dir)
        if url_dep.verify():
            processed.append(url_dep)
        log.info("-- Finished processing %s", url_dep.name)
    ...
    if pypi_deps:
        ...
        if files_to_download:
            ...
            headers = headers_by_url if headers_by_url else None
            asyncio.run(
                async_download_files(
                    files_to_download, get_config().runtime.concurrency_limit, headers=headers
                )
            )
...
class PipLockfile(ABC):
    ...
    def trusted_hosts(self) -> set[str]:
        """Return set of trusted hosts (--trusted-host). Defaults to empty."""
        return set()
...
_LOCKFILE_TYPES: dict[PythonPackagingTool, type[PipLockfile]] = {
    ...
    # PythonPackagingTool.PYLOCK: PylockLockfile,  # Added in commit 4
}
